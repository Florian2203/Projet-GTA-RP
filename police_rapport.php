...
if (isset($_SESSION['user_id'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        echo $titre, $auteur_prenom, $contenu, $auteur_nom;
        // Préparez et exécutez la requête SQL pour insérer l'article
        $sql = "INSERT INTO rapports (titre, contenu, auteur_nom, auteur_prenom, date_publication) VALUES (?, ?, ?, ?, NOW())";
        ...
        if ($stmt->affected_rows > 0) {
           header("Location:listedesrapports.php");
        } else {
            echo "Erreur lors de l'enregistrement de l'enquête : " . $conn->error;
        }
    } else {
    ...
    echo "Vous devez être connecté pour soumettre une enquête.";
}
